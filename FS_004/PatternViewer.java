...
//      angle_progress = angle_progress + delta_angle*(100*(x_progress / width)*(x_progress / width))/10;
  ...
  public int pos_polynonmial_ctape(float x, float y, float from_angle, float to_angle, float delta_mag, float strip_width, double order, float test_width) { 
	    ...
	    for(int i = 0; ((x_progress - x) < strip_width) && ((x_progress - x) < test_width); i++) {
	    	...
	    	angle_progress = (from_angle - to_angle) * ((float) Math.pow(t_progress - 1, order)) + to_angle;
  ...
  public int neg_polynonmial_ctape(float x, float y, float from_angle, float to_angle, float delta_mag, float strip_width, double order, float test_width) { 
	    ...
	    for(int i = 0; ((x_progress - x) < strip_width) && ((x_progress - x) < test_width); i++) {
	    	...
	    	// Use a Polynomial Curve to calculate angle
	    	angle_progress = ((180-from_angle) - (180-to_angle)) * ((float) Math.pow(t_progress - 1, order)) + (180-to_angle);
...
//
  ...
  public int gradient_ctape(float x, float y, float from_angle, float to_angle, float delta_mag, float strip_width, PImage img, int the_y) {
	  int num_wraps = 0;
	  
	  segmentBegin(false);    
	  if(from_angle > 90) {
		  num_wraps = neg_gradient_ctape(x, y, from_angle, to_angle, delta_mag, strip_width, img, the_y);
	  } else {
		  num_wraps = pos_gradient_ctape(x, y, from_angle, to_angle, delta_mag, strip_width, img, the_y);
	  }
	  segmentEnd();
	  return num_wraps;
  }
	  
  public int pos_gradient_ctape(float x, float y, float from_angle, float to_angle, float delta_mag, float strip_width, PImage img, int the_y) { 
	    int num_wraps = 0;
	    float t_progress = 0.0f; // parameter: 0-2.0f (1.0f in the middle of the piece)
	    float angle_progress = from_angle;
	    ...
	    // Tape the length of the segment
	    for(int i = 0; ((x_progress - x) < strip_width); i++) {
	    	// Calculate parameter t based on x_progress:
	    	t_progress = ((x_progress - x) / strip_width) * 2.0f;
	    	
	    	// Use Gradient Map to Calculate the Angle
	    	// angle_progress = (from_angle - to_angle) * ((float) Math.pow(t_progress - 1, order)) + to_angle;
               gradVal = (int) (red(img.pixels[(the_y)*img.width+ (int) (x_progress)]));
                
                if(from_angle < to_angle) {
          	  angle_progress = map(gradVal, 0, 255, from_angle, to_angle);
                } else {
          	  angle_progress = map(gradVal, 0, 255, to_angle, from_angle);                  
                }
		// println("Polynomial Curve segment [" + i + "] t: " + t_progress + " - angle_progress: " + angle_progress);
  ...
  public int neg_gradient_ctape(float x, float y, float from_angle, float to_angle, float delta_mag, float strip_width, PImage img, int the_y) { 
	    int num_wraps = 0;
	    float t_progress = 0.0f; // parameter: 0-2.0f (1.0f in the middle of the piece)
	    float angle_progress = 180-from_angle;
	    ...
	    for(int i = 0; ((x_progress - x) < strip_width); i++) {
	    	...
	    	// angle_progress = (from_angle - to_angle) * ((float) Math.pow(t_progress - 1, order)) + to_angle;
                gradVal = (int) (red(img.pixels[(the_y)*img.width + (int) (x_progress)]));
                if((180 - from_angle) < (180 - to_angle)) {
          	  angle_progress = map(gradVal, 0, 255, (180 - from_angle), (180 - to_angle));
                } else {
          	  angle_progress = map(gradVal, 0, 255, (180 - to_angle), (180 - from_angle));                  
                }
		// println("Polynomial Curve segment [" + i + "] t: " + t_progress + " - angle_progress: " + angle_progress);
